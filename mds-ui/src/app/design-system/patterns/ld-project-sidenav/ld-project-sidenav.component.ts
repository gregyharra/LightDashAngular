...
export interface LdProjectSidenavItem {
  readonly id: string;
...
@Component({
  selector: 'ld-project-sidenav',
  imports: [RouterLink, MatIconModule],
  template: `
    <a
      class="ld-project-sidenav__home"
      routerLink="/projects"
    ...
    <nav
      class="page-sidebar__nav ld-project-sidenav"
    ...
    >
      @for (item of items(); track item.id) {
        ...
        >
          ...
          <span class="ld-project-sidenav__label">{{ item.label }}</span>
        </a>
      }
    </nav>
  `,
  styles: `
    ...
    .ld-project-sidenav__label {
      min-width: 0;
      overflow: hidden;
      text-overflow: ellipsis;
    }

    :host-context(.page-sidebar--collapsed) .ld-project-sidenav__home {
      display: none;
    }

    :host-context(.page-sidebar--collapsed) .ld-project-sidenav__label {
      position: absolute;
      width: 1px;
      height: 1px;
      padding: 0;
...
export class LdProjectSidenavComponent {
  readonly projectUuid = input.required<string>();
  readonly active = input.required<string>();
  readonly items = input.required<readonly LdProjectSidenavItem[]>();

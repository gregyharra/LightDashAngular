...
export class ResizableSidebarDirective implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const resizeHandle = this.renderer.createElement('div') as HTMLElement;
    this.resizeHandle = resizeHandle;
    this.renderer.addClass(resizeHandle, 'page-sidebar__resize-handle');
    this.renderer.setAttribute(resizeHandle, 'role', 'separator');
    this.renderer.setAttribute(resizeHandle, 'aria-orientation', 'vertical');
    this.renderer.setAttribute(
      resizeHandle,
      'aria-label',
      this.translate.instant('sidebar.resize'),
    );
    this.renderer.setAttribute(resizeHandle, 'tabindex', '0');
    this.renderer.listen(resizeHandle, 'pointerdown', (event: PointerEvent) =>
      this.onPointerDown(event, resizeHandle),
    );

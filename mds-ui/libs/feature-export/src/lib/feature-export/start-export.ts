...
export function startExport(opts: StartExportOptions): void {
  const result$ = opts.skipDialog
    ...
    : toDialogResult(
        opts.dialog.open(opts.dialogComponent, {
          data: {
            format: opts.format,
...
function toDialogResult(opened: unknown): Observable<ExportDialogResult> {
  if (isObservable(opened)) {
    return opened as Observable<ExportDialogResult>;
  }
  const ref = opened as { afterClosed?: () => Observable<ExportDialogResult> };

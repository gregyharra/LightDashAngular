...
class User(Base):
    ...
    organization_uuid: Mapped[uuid.UUID] = mapped_column(
...
class Space(Base):
    ...
    parent_space_uuid: Mapped[uuid.UUID | None] = mapped_column(Uuid, nullable=True)
...
class Dashboard(Base):
    ...
    organization_uuid: Mapped[uuid.UUID] = mapped_column(Uuid, nullable=False)
    space_uuid: Mapped[uuid.UUID] = mapped_column(Uuid, ForeignKey("spaces.uuid"))
    name: Mapped[str] = mapped_column(String(255), nullable=False)
    description: Mapped[str | None] = mapped_column(Text, nullable=True)
    slug: Mapped[str] = mapped_column(String(255), nullable=False)
    dashboard_version_id: Mapped[int] = mapped_column(Integer, default=1)
    version_uuid: Mapped[uuid.UUID] = mapped_column(Uuid, nullable=False)
    views: Mapped[int] = mapped_column(Integer, default=0)
    first_viewed_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    updated_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), onupdate=func.now()
    )
    updated_by_user_uuid: Mapped[uuid.UUID | None] = mapped_column(Uuid, ForeignKey("users.uuid"))
    ...
    color_palette_uuid: Mapped[uuid.UUID | None] = mapped_column(Uuid, nullable=True)
    verification: Mapped[dict | None] = mapped_column(JSON, nullable=True)
    pinned_list_uuid: Mapped[uuid.UUID | None] = mapped_column(Uuid, nullable=True)

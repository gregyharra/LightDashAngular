...
class User(Base):
    ...
    __table_args__ = (
        Index(
            ...
            "oidc_sub",
            unique=True,
            postgresql_where=text("oidc_issuer IS NOT NULL AND oidc_sub IS NOT NULL"),
            sqlite_where=text("oidc_issuer IS NOT NULL AND oidc_sub IS NOT NULL"),
        ),
    )
...
class ModelJoin(Base):
    """Custom explore join overlay for a source dbt model."""

    __tablename__ = "model_joins"

    uuid: Mapped[uuid_lib.UUID] = mapped_column(Uuid, primary_key=True)
    project_uuid: Mapped[uuid_lib.UUID] = mapped_column(
        Uuid, ForeignKey("projects.uuid", ondelete="CASCADE"), nullable=False
    )
    source_dbt_unique_id: Mapped[str] = mapped_column(String(512), nullable=False)
    source_model_name: Mapped[str] = mapped_column(String(255), nullable=False)
    source_column: Mapped[str] = mapped_column(String(255), nullable=False)
    target_dbt_unique_id: Mapped[str] = mapped_column(String(512), nullable=False)
    target_model_name: Mapped[str] = mapped_column(String(255), nullable=False)
    target_column: Mapped[str] = mapped_column(String(255), nullable=False)
    join_type: Mapped[str] = mapped_column(String(32), default="left", nullable=False)
